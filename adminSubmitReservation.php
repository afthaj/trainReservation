...
<head>
<title>Submit Reservation | TrainReserve Admin</title>
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!$error) {
		...
		if (!empty($class) && !empty($number_of_passengers)) {
			...
			mysqli_query($dbc, $query);
			// Confirm success with the user
			$error_msg = 'The Reservation has been successfully submitted. Would you like to <a href="adminEditProfile.php">view</a> your profile and check your reservations? Alternatively, you can <a href="adminCheckTrainSchedule.php">make another reservation</a>';
			mysqli_close($dbc);
			} else {
				$error_msg = 'You must enter a Class and a specific amount of passengers in the relevant fields.';
				}
...
?>
		...
		<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="adminSubmitReservationForm" id="adminSubmitReservationForm" onsubmit="return submitReservation(this.form)">
        ...
        <ol>
          ...
          <textarea rows="8" cols="50" id="otherinformation" name="otherinformation" readonly="readonly"><?php if (!empty($other_information)) echo $other_information; ?></textarea>
          <br />
          </li>
        </ol>
        </fieldset>
        ...
        <input type="submit" class="submit" name="submit" value="Submit Reservation" />
        </fieldset>
