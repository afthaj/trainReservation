...
<head>
<title>Add New Admin | TrainReserve</title>
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!$error) {
      if (!empty($first_name) && !empty($last_name) && !empty($gender) && !empty($dob) && !empty($address) && ($password == $retype_password)) {
          $query = "INSERT INTO admin_user (username, password, joinDate, firstname, othernames, lastname, gender, dateofbirth, age, address, telephoneNumber, emailAddress) VALUES ('$username', '$password', NOW(), '$first_name', '$other_names', '$last_name', '$gender', '$dob', '$age', '$address', '$telnumber', '$email')";
		  
        mysqli_query($dbc, $query);

        // Confirm success with the user
        $error_msg = 'New Admin has successfully been added.';
        mysqli_close($dbc);
      } else {
...
?>
		...
		<?php
			...
			echo '<p>' . $error_msg . '</p>';
		?>
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="adminAddNewAdminForm" id="adminAddNewAdminForm" onsubmit="return validateAdminAddNewAdmin(this.form)">
        ...
        <fieldset class="submit">
          <input type="submit" class="submit" name="submit" value="Add New Admin">
        </fieldset>
