...
<head>
<title>Edit Profile | TrainReserve</title>
...
?>
		...
		<?php
			...
			if (isset($_SESSION['passengerID'])) {
			echo '<p>You are logged in as ' . $_SESSION['firstname'] . ' ' . $_SESSION['lastname'] . ' - <a href="logout.php">Log Out</a></p>';
			if(!empty($error_msg)){echo '<p>' . $error_msg . '</p>';}
		?>
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="editProfileForm" id="editProfileForm" onsubmit="return validateEditProfile(this.form)">
        
        <fieldset>
        <legend><span>Login Details</span></legend>
        <ol>
          <li>If you want to change your password click <a href="changePassword.php">here</a></li>
        </ol>
        </fieldset>
        ...
        <?php
			...
			$query2 = "SELECT r.reservationID, r.trainNo, r.class, r.noOfPassengers, r.dateSubmitted, t.trainName, t.departureStation, t.departureTime, t.arrivalStation, t.arrivalTime, t.frequency, t.otherInformation FROM reservation AS r, train_schedule AS t WHERE t.trainNo = r.trainNo AND r.passengerID = '" . $_SESSION['passengerID'] . "'";
