			...
			if (isset($_POST['submit'])) {
			  ...
			  $password = mysqli_real_escape_string($dbc, trim($_POST['password']));
						  ...
						  if (!empty($username) && !empty($password)) {
							...
							$query = "SELECT passengerID, username, firstname, lastname FROM passenger WHERE username = '$username' AND password = '$password'";
							$data = mysqli_query($dbc, $query);
										...
										if (mysqli_num_rows($data) == 1) {
										  ...
										  setcookie('passengerID', $row['passengerID'], time() + (60 * 60 * 24 * 30));    // expires in 30 days
...
<title>User Login | TrainReserve</title>
  ...
  <div class="bodyContainer">
    ...
    <div class="mainContentContainer">
	  ...
	  <?php
          ...
          if (empty($_SESSION['passengerID'])) {
          echo '<p>' . $error_msg . '</p>';
      ?>
       <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="userLoginForm" name="userLoginForm" onsubmit="return validateLogin(this.form)">
          ...
          <ol>
            ...
            <input type="password" id="password" onblur="validateNonEmpty(this, document.getElementById('password_help'))" name="password" size="20"/>
